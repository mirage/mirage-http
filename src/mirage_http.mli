module type S = sig
  ...
  module Request : sig
    ...
    val v : (* paramètres par défaut là dedans (header Host par exemple) *)
  ...
  module Client (CON: Conduit_mirage.S): sig
    ...
    val connect :  Resolver_lwt.t
                -> CON.t
                -> t io
    end

  module Server (CON: Conduit_mirage.S) : sig
    module Channel : Mirage_channel_lwt.S
    ...
    val create_connection_handler :  request_handler
                                  -> error_handler
                                  -> connection_handler io
