...
let stream_to_body body =
  ...
  |> Lwt_stream.map (fun (x,ofs,len) -> Cstruct.sub x ofs len |> Cstruct.to_string)
...
module Response = struct
  ...
  let with_headers x headers = {x with resp= {x.resp with Cohttp.Response.headers}}
...
module Server (CON: Conduit_mirage.S) = struct
  ...
  module Channel = Cohttp_server.Channel
  ...
  type response = [ `Response of resp
  ...
  let to_cohttp_request_handler (request_handler : request_handler) =
    fun (conn : Cohttp_server.conn) (request : Cohttp.Request.t) (body : Cohttp_lwt.Body.t) : Cohttp_server.response_action Lwt.t ->
      let req = { req = request
                ; body = Some (body_to_stream body)}
      in
      request_handler (req, conn)
      >>= map_to_response_action


  let create_connection_handler request_handler _error_handler =
    let callback = to_cohttp_request_handler request_handler in
    Lwt.return (
      Cohttp_mirage.Server_with_conduit.make_response_action
        ~callback
        ()
    )
