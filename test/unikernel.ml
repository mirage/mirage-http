...
module Make (RES : Resolver_lwt.S) (CON : Conduit_mirage.S) = struct
    ...
    let start resolver conduit =
        HTTP_Client.connect resolver conduit >>= fun t ->
        ...
        |> Lwt_stream.iter (fun (x,ofs,len) -> Printf.printf "Line: %s\n" (Cstruct.to_string (Cstruct.sub x ofs len)))
...
module MakeServer (CON : Conduit_mirage.S) = struct
    ...
    let request_handler _ =
        ...
        let resp = HTTP.Response.v ~body HTTP.HTTP.Headers.empty 200 in
        let response = `Response resp in
        Lwt.return response

    let start conduit =
        Server.connect conduit >>= fun t ->
        Server.create_connection_handler request_handler error_handler >>= fun conn_handler ->
        Server.listen t (`TCP 8080) conn_handler
end
